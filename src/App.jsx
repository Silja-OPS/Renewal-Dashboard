...
export default function App() {
  ...
  const subMonthlyFiltered = subMonthly.map(m => {
    ...
    const count    = statusFilter === "ACTIVE"   ? active
                   : statusFilter === "INACTIVE" ? inactive
                   : total;
    return { ...m, month: m.month.replace(" 20"," '"), new_subscribers: count, active, inactive, total };
  });
      ...
      <div style={{ padding:"24px" }}>
        ...
        {tab==="analytics" && (() => {
          ...
          const subTrend = subMonthly.map(m => {
            ...
            return { month_key:m.month_key, month:m.month.replace(" 20"," '"), new_subscribers:count };
          });
          let _cum = _preLaunch;
        ...
        {tab==="analytics" && (() => {
          ...
          // PDF-style diamond dot
          const DiamondDot = ({ cx, cy, value, fill }) => {
            if (!cx||!cy||!value) return null;
            const s = 6;
            return <polygon points={`${cx},${cy-s} ${cx+s},${cy} ${cx},${cy+s} ${cx-s},${cy}`} fill={fill||"#1a3a8f"} stroke="#fff" strokeWidth={1.5} />;
          };

          // Custom label above each point
          const TopLabel = ({ x, y, value }) => {
            if (!value && value !== 0) return null;
            return <text x={x} y={y-10} textAnchor="middle" fontSize={10} fontWeight="700" fill="#1a3a8f">{value}</text>;
          };

          // Reusable DataTable below each chart
          const DataTable = ({ rows, headers, colors }) => (
            <div style={{ overflowX:"auto", marginTop:16, border:"1px solid #b8cce4", borderRadius:4 }}>
              <table style={{ borderCollapse:"collapse", fontSize:11, minWidth:"100%", width:"100%" }}>
                <tbody>
                  {rows.map((row, ri) => (
                    <tr key={ri}>
                      <td style={{ padding:"6px 12px", fontWeight:700, background:"#dce6f1", color:"#1a3a8f",
                        whiteSpace:"nowrap", borderRight:"1px solid #b8cce4", minWidth:180, fontSize:11 }}>
                        {headers[ri]}
                      </td>
                      {row.map((val, ci) => (
                        <td key={ci} style={{
                          padding:"6px 10px", textAlign:"center",
                          background: ri%2===0 ? "#fff" : "#dce6f1",
                          border:"1px solid #b8cce4", fontWeight:500,
                          color: colors?.[ri] || "#1e293b", whiteSpace:"nowrap", fontSize:11
                        }}>{val ?? "—"}</td>
                      ))}
                    </tr>
                  ))}
          ...
          // Chart wrapper
          const ChartCard = ({ title, subtitle, children, tableRows, tableHeaders, tableColors }) => (
            <div style={{ background:"#fff", borderRadius:12, padding:24, marginBottom:20,
              boxShadow:"0 1px 4px rgba(0,0,0,0.08)", border:"0.5px solid #e2e8f0" }}>
              <h3 style={{ margin:"0 0 2px", fontSize:15, fontWeight:700, color:"#1e293b", textAlign:"center" }}>{title}</h3>
              {subtitle && <p style={{ margin:"0 0 16px", fontSize:11, color:"#64748b", textAlign:"center" }}>{subtitle}</p>}
              {children}
              <DataTable rows={tableRows} headers={tableHeaders} colors={tableColors} />
            </div>
          );

          const subLabel = statusFilter==="ACTIVE"?"Active Subscribers":statusFilter==="INACTIVE"?"Inactive Subscribers":"All Subscribers";
          const regionLabel = countryFilter==="ALL"?"All Regions":(COUNTRY_NAMES[countryFilter]||countryFilter);
          ...
          return (
            <div>
              ...
              <ChartCard
                title="Active Subscribers vs Month"
                subtitle={`${regionLabel} · ${subLabel}`}
                tableHeaders={[`Total ${subLabel} (${regionLabel})`, "Month"]}
                tableRows={[
                  subTrend.map(m => m.total_active),
                  subTrend.map(m => m.month),
                ]}
                tableColors={["#1a3a8f", "#374151"]}
              >
                <ResponsiveContainer width="100%" height={280}>
                  <AreaChart data={subTrend} margin={{ top:30, right:20, left:10, bottom:10 }}>
                    <defs>
                      <linearGradient id="grad1" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="0%" stopColor="#7b9fd4" stopOpacity={0.8} />
                        <stop offset="100%" stopColor="#7b9fd4" stopOpacity={0.2} />
                      </linearGradient>
                    </defs>
                    <CartesianGrid strokeDasharray="" stroke="#e2e8f0" vertical={false} />
                    <XAxis dataKey="month" tick={{ fontSize:11, fontWeight:600, fill:"#374151" }} axisLine={false} tickLine={false} />
                    <YAxis tick={{ fontSize:10, fill:"#94a3b8" }} axisLine={false} tickLine={false} />
                    <Tooltip formatter={(v) => [v, subLabel]} />
                    <Area type="linear" dataKey="total_active" name={subLabel}
                      stroke="#1a3a8f" strokeWidth={2.5} fill="url(#grad1)"
                      dot={<DiamondDot fill="#1a3a8f" />} activeDot={{ r:6, fill:"#1a3a8f" }}>
                      <LabelList dataKey="total_active" content={<TopLabel />} />
                    </Area>
                  </AreaChart>
                </ResponsiveContainer>
              </ChartCard>
              ...
              <ChartCard
                title="New Subscribers vs Month"
                subtitle={`${regionLabel} · New ${subLabel} registrations per month`}
                tableHeaders={[`New ${subLabel}`, "Month"]}
                tableRows={[
                  subTrend.map(m => m.new_subscribers),
                  subTrend.map(m => m.month),
                ]}
                tableColors={["#1a3a8f", "#374151"]}
              >
                <ResponsiveContainer width="100%" height={280}>
                  <AreaChart data={subTrend} margin={{ top:30, right:20, left:10, bottom:10 }}>
                    ...
                    <Tooltip formatter={(v) => [v, `New ${subLabel}`]} />
                    <Area type="linear" dataKey="new_subscribers" name={`New ${subLabel}`}
                      stroke="#1a3a8f" strokeWidth={2.5} fill="url(#grad2)"
                      dot={<DiamondDot fill="#1a3a8f" />} activeDot={{ r:6, fill:"#1a3a8f" }}>
                      <LabelList dataKey="new_subscribers" content={<TopLabel />} />
                    </Area>
                  </AreaChart>
                </ResponsiveContainer>
              </ChartCard>

              {/* ── Chart 3: Transaction Count by Gateway ── */}
              <ChartCard
                title="Transaction Count — Metrobank, PIN-Based & In-App"
                subtitle="SUCCESS transactions by payment gateway per month"
                tableHeaders={["Metrobank", "Pin Based", "In App", "Month"]}
                tableRows={[
                  trend.map(m => m.METROBANK||0),
                  trend.map(m => m.PINBASED||0),
                  trend.map(m => m.INAPP||0),
                  trend.map(m => m.month),
                ]}
                tableColors={["#1a56db","#e3a008","#0e9f6e","#374151"]}
              >
                ...
                <ResponsiveContainer width="100%" height={280}>
                  <ComposedChart data={trend} margin={{ top:30, right:20, left:10, bottom:10 }}>
                    <CartesianGrid strokeDasharray="" stroke="#e2e8f0" vertical={false} />
                    <XAxis dataKey="month" tick={{ fontSize:11, fontWeight:600, fill:"#374151" }} axisLine={false} tickLine={false} />
                    <YAxis tick={{ fontSize:10, fill:"#94a3b8" }} axisLine={false} tickLine={false} />
                    <Tooltip />
                    ...
                    <Line type="linear" dataKey="total" name="Total" stroke="#f97316" strokeWidth={2.5}
                      dot={<DiamondDot fill="#f97316" />}>
                      <LabelList dataKey="total" content={({ x,y,value }) => value ? <text x={x} y={y-10} textAnchor="middle" fontSize={10} fontWeight="700" fill="#f97316">{value}</text> : null} />
                    </Line>
                  </ComposedChart>
                </ResponsiveContainer>
              </ChartCard>

              {/* ── Chart 4: Month Renewal ── */}
              <ChartCard
                title="Month Renewal"
                subtitle="Customers who purchased again the subscriptions"
                tableHeaders={["Renewals", "Month"]}
                tableRows={[
                  trend.map(m => m.renewals||0),
                  trend.map(m => m.month),
                ]}
                tableColors={["#7c3aed","#374151"]}
              >
                <ResponsiveContainer width="100%" height={280}>
                  <AreaChart data={trend} margin={{ top:30, right:20, left:10, bottom:10 }}>
                    <defs>
                    ...
                    <XAxis dataKey="month" tick={{ fontSize:11, fontWeight:600, fill:"#374151" }} axisLine={false} tickLine={false} />
                    <YAxis tick={{ fontSize:10, fill:"#94a3b8" }} axisLine={false} tickLine={false} />
                    <Tooltip formatter={(v) => [v, "Renewals"]} />
                    <Area type="linear" dataKey="renewals" name="Renewals"
                      stroke="#6d28d9" strokeWidth={2.5} fill="url(#grad4)"
                      dot={<DiamondDot fill="#6d28d9" />} activeDot={{ r:6, fill:"#6d28d9" }}>
                      <LabelList dataKey="renewals" content={({ x,y,value }) => value ? <text x={x} y={y-10} textAnchor="middle" fontSize={10} fontWeight="700" fill="#6d28d9">{value}</text> : null} />
                    </Area>
                  </AreaChart>
                </ResponsiveContainer>
              </ChartCard>
        ...
        {/* ══ ANALYTICS ══ */}
        {tab==="analytics" && (() => {
          // Build subscriber trend filtered by country
          const filterCC = countryFilter === "ALL" ? "ALL" : countryFilter;
          const subTrend = subMonthly.map(m => {
            const total = m[`${filterCC}__TOTAL`] || 0;
            const active = m[`${filterCC}__ACTIVE`] || 0;
            const verify = m[`${filterCC}__VERIFY`] || 0;
            return { month_key: m.month_key, month: m.month.replace(" 20"," '"), new_subscribers: total, active, verify };
          });
          let cum = 0;
          subTrend.forEach(m => { cum += m.new_subscribers; m.total_active = cum; });

          const DiamondDot = (props) => {
            const { cx, cy, fill } = props;
            if (!cx || !cy) return null;
            const s = 5;
            return <polygon points={`${cx},${cy-s} ${cx+s},${cy} ${cx},${cy+s} ${cx-s},${cy}`} fill={fill} stroke="#fff" strokeWidth={1} />;
          };

          const TableBelow = ({ rows, headers }) => (
            <div style={{ overflowX:"auto", marginTop:14 }}>
              <table style={{ borderCollapse:"collapse", fontSize:11, minWidth:"100%" }}>
                <tbody>
                  {rows.map((row,ri)=>(
                    <tr key={ri}>
                      <td style={{ padding:"5px 10px", fontWeight:500, color:"var(--text-secondary)", whiteSpace:"nowrap", borderRight:"0.5px solid var(--border)", background:"var(--surface-1)", minWidth:160 }}>{headers[ri]}</td>
                      {row.map((val,ci)=>(
                        <td key={ci} style={{ padding:"5px 8px", textAlign:"center", border:"0.5px solid var(--border)", fontWeight:500, color:"var(--text-primary)", whiteSpace:"nowrap", background: ri%2===0?"var(--surface-2)":"var(--surface-1)" }}>{val ?? "—"}</td>
                      ))}
                    </tr>
                  ))}
                </tbody>
          ...
          return (
            <div>
              {/* Chart 1 — Active Subscribers vs Month */}
              <div style={{ background:"var(--surface-2)", border:"0.5px solid var(--border)", borderRadius:12, padding:20, marginBottom:16 }}>
                <h3 style={{ margin:"0 0 2px", fontSize:14, fontWeight:500 }}>Active Subscribers vs Month</h3>
                <p style={{ margin:"0 0 14px", fontSize:11, color:"var(--text-muted)" }}>
                  {countryFilter==="ALL" ? "All regions" : (COUNTRY_NAMES[countryFilter]||countryFilter)} ·{" "}
                  {statusFilter==="ACTIVE" ? "Active subscribers only" : statusFilter==="INACTIVE" ? "Inactive subscribers only" : "All subscribers"}
                </p>
                <ResponsiveContainer width="100%" height={250}>
                  <AreaChart data={subTrend} margin={{ top:20, right:10, left:0, bottom:0 }}>
                    <defs>
                      <linearGradient id="grad1" x1="0" y1="0" x2="0" y2="1">
                    ...
                    <Tooltip />
                    <Area type="monotone" dataKey="total_active" name="Total Active"
                      stroke="#3730a3" strokeWidth={2.5} fill="url(#grad1)"
                      dot={<DiamondDot fill="#1e3a8a" />} activeDot={{ r:5 }}>
                      <LabelList dataKey="total_active" position="top" style={{ fontSize:9, fontWeight:700, fill:"#1e3a8a" }} />
                    </Area>
                  </AreaChart>
                </ResponsiveContainer>
                <TableBelow
                  headers={["Total Active Subscribers", "Month"]}
                  rows={[
                    subTrend.map(m => m.total_active),
                    subTrend.map(m => m.month),
                  ]}
              ...
              {/* Chart 2 — New Subscribers per Month */}
              <div style={{ background:"var(--surface-2)", border:"0.5px solid var(--border)", borderRadius:12, padding:20, marginBottom:16 }}>
                <h3 style={{ margin:"0 0 2px", fontSize:14, fontWeight:500 }}>New Subscribers per Month</h3>
                <p style={{ margin:"0 0 14px", fontSize:11, color:"var(--text-muted)" }}>
                  {countryFilter==="ALL" ? "All regions" : (COUNTRY_NAMES[countryFilter]||countryFilter)} ·{" "}
                  {statusFilter==="ACTIVE" ? "Active only" : statusFilter==="INACTIVE" ? "Inactive only" : "All subscribers"}
                </p>
                <ResponsiveContainer width="100%" height={250}>
                  <AreaChart data={subTrend} margin={{ top:20, right:10, left:0, bottom:0 }}>
                    <defs>
                    ...
                    <Area type="monotone" dataKey="new_subscribers" name="New Subscribers"
                      stroke="#0369a1" strokeWidth={2.5} fill="url(#grad2)"
                      dot={<DiamondDot fill="#0369a1" />} activeDot={{ r:5 }}>
                      <LabelList dataKey="new_subscribers" position="top" style={{ fontSize:9, fontWeight:700, fill:"#0369a1" }} />
                    </Area>
                  </AreaChart>
                </ResponsiveContainer>
                <TableBelow
                  headers={["New Subscribers", "Month"]}
                  rows={[
                    subTrend.map(m => m.new_subscribers),
                    subTrend.map(m => m.month),
                  ]}
              ...
              {/* Chart 3 — Transactions by Gateway */}
              <div style={{ background:"var(--surface-2)", border:"0.5px solid var(--border)", borderRadius:12, padding:20, marginBottom:16 }}>
                <h3 style={{ margin:"0 0 2px", fontSize:14, fontWeight:500 }}>Total Transaction — Metrobank, PIN-Based & In-App</h3>
                <p style={{ margin:"0 0 14px", fontSize:11, color:"var(--text-muted)" }}>SUCCESS transactions by gateway per month</p>
                <ResponsiveContainer width="100%" height={250}>
                  <ComposedChart data={trend} margin={{ top:20, right:10, left:0, bottom:0 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="var(--border)" />
                    <XAxis dataKey="month" tick={{ fontSize:10 }} />
                    <YAxis tick={{ fontSize:10 }} allowDecimals={false} />
                    <Tooltip />
                    <Legend wrapperStyle={{ fontSize:11 }} />
                    <Bar dataKey="METROBANK" name="Metrobank" stackId="g" fill={GW_COLORS.METROBANK}>
                      <LabelList dataKey="METROBANK" position="inside" style={{ fontSize:8, fill:"#fff", fontWeight:700 }} />
                    </Bar>
                    <Bar dataKey="PINBASED" name="PIN-Based" stackId="g" fill={GW_COLORS.PINBASED}>
                      <LabelList dataKey="PINBASED" position="inside" style={{ fontSize:8, fill:"#fff", fontWeight:700 }} />
                    </Bar>
                    <Bar dataKey="INAPP" name="In-App" stackId="g" fill={GW_COLORS.INAPP} radius={[4,4,0,0]}>
                      <LabelList dataKey="INAPP" position="inside" style={{ fontSize:8, fill:"#fff", fontWeight:700 }} />
                    </Bar>
                    <Line type="monotone" dataKey="total" name="Total" stroke="#f97316" strokeWidth={2}
                      dot={<DiamondDot fill="#f97316" />}>
                      <LabelList dataKey="total" position="top" style={{ fontSize:9, fontWeight:700, fill:"#f97316" }} />
                    </Line>
                  </ComposedChart>
                </ResponsiveContainer>
                <TableBelow
                  headers={["Total Transaction", "Metrobank", "PIN-Based", "In-App", "Month"]}
                  rows={[
                    trend.map(m => m.total),
                    trend.map(m => m.METROBANK),
                    trend.map(m => m.PINBASED),
                    trend.map(m => m.INAPP),
                    trend.map(m => m.month),
                  ]}
                />
              ...
              <div style={{ background:"var(--surface-2)", border:"0.5px solid var(--border)", borderRadius:12, padding:20, marginBottom:16 }}>
                <h3 style={{ margin:"0 0 2px", fontSize:14, fontWeight:500 }}>Month Renewal</h3>
                <p style={{ margin:"0 0 14px", fontSize:11, color:"var(--text-muted)" }}>Customers who renewed their subscription each month</p>
                <ResponsiveContainer width="100%" height={250}>
                  <AreaChart data={trend} margin={{ top:20, right:10, left:0, bottom:0 }}>
                    ...
                    <YAxis tick={{ fontSize:10 }} allowDecimals={false} />
                    <Tooltip />
                    <Area type="monotone" dataKey="renewals" name="Renewals"
                      stroke="#6d28d9" strokeWidth={2.5} fill="url(#grad4)"
                      dot={<DiamondDot fill="#6d28d9" />} activeDot={{ r:5 }}>
                      <LabelList dataKey="renewals" position="top" style={{ fontSize:9, fontWeight:700, fill:"#6d28d9" }} />
                    </Area>
                  </AreaChart>
                </ResponsiveContainer>
                <TableBelow
                  headers={["Renewals", "Month"]}
                  rows={[
                    trend.map(m => m.renewals),
                    trend.map(m => m.month),
                  ]}
                />
              </div>
