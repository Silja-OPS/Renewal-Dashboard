...
export default function App() {
  ...
  // ── Correct subscriber counts: directly from subRows by current status + country filter ──
  const filterCC = countryFilter === "ALL" ? "ALL" : countryFilter;
  const ccMatch  = r => countryFilter === "ALL" || (r.subscribercountry || "PH") === countryFilter;
  // statusMatch: respect the status dropdown filter
  const stMatch  = r => {
    if (statusFilter === "ALL")      return true;
    if (statusFilter === "ACTIVE")   return r.subscriberstatus === "ACTIVE";
    if (statusFilter === "INACTIVE") return r.subscriberstatus === "VERIFY" || r.subscriberstatus === "SUSPENDED";
    return true;
  };

  // Always show all 3 status counts for the selected country (for stat cards overview)
  const totalActive    = sRows.filter(r => r.subscriberstatus === "ACTIVE"    && ccMatch(r)).length;
  const totalInactive  = sRows.filter(r => r.subscriberstatus === "VERIFY"    && ccMatch(r)).length;
  const totalSuspended = sRows.filter(r => r.subscriberstatus === "SUSPENDED" && ccMatch(r)).length;
  const totalAllSubs   = sRows.filter(r => ccMatch(r)).length;
  // filteredSubCount: what the current status filter selection shows
  const filteredSubCount = sRows.filter(r => ccMatch(r) && stMatch(r)).length;

  // ── Monthly breakdown: users grouped by registration month, using CURRENT status ──
  // Each bar = how many users registered that month who are currently active/inactive/suspended
      ...
      <div style={{ padding:"24px" }}>
        ...
        {tab==="subscribers" && (
          <div>
            ...
            {statusFilter !== "ALL" && (
              <div style={{ background: statusFilter==="ACTIVE" ? "#d1fae5" : "#fef3c7",
                border: `1px solid ${statusFilter==="ACTIVE" ? "#0e9f6e" : "#f59e0b"}`,
                borderRadius:10, padding:"10px 16px", marginBottom:16, fontSize:13,
                color: statusFilter==="ACTIVE" ? "#065f46" : "#92400e", fontWeight:600 }}>
                {statusFilter==="ACTIVE" ? "✅" : "⚠️"} Showing{" "}
                <b>{statusFilter==="ACTIVE" ? "Active" : "Inactive (VERIFY + Suspended)"}</b> subscribers only
                {countryFilter !== "ALL" && ` · ${COUNTRY_NAMES[countryFilter]||countryFilter} region`}
                {" "}— <b>{fmt(filteredSubCount)}</b> users
              </div>
            )}
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))", gap:14, marginBottom:22 }}>
              <StatCard label="Total Subscribers" value={fmt(totalAllSubs)}
                sub={countryFilter==="ALL"?"All regions":(COUNTRY_NAMES[countryFilter]||countryFilter)}
                color="#1a56db" icon="👥" />
              <StatCard label="Active" value={fmt(totalActive)}
                sub={`${pct(totalActive,totalAllSubs)}% of total`}
                color={statusFilter==="ACTIVE" ? "#0e9f6e" : "#94a3b8"} icon="✅" />
              <StatCard label="VERIFY (Inactive)" value={fmt(totalInactive)}
                sub={`${pct(totalInactive,totalAllSubs)}% of total`}
                color={statusFilter==="INACTIVE" ? "#e3a008" : "#94a3b8"} icon="🔍" />
              <StatCard label="Suspended" value={fmt(totalSuspended)}
                sub={`${pct(totalSuspended,totalAllSubs)}% of total`}
                color={statusFilter==="INACTIVE" ? "#e02424" : "#94a3b8"} icon="🚫" />
            </div>

            {/* Active/Inactive/Suspended subscribers — by registration month */}
            <div style={{ background:"#fff", borderRadius:12, padding:20, marginBottom:18, boxShadow:"0 1px 4px rgba(0,0,0,0.06)" }}>
              <h3 style={{ margin:"0 0 4px", fontSize:14, fontWeight:700 }}>
                {statusFilter === "ACTIVE"   ? "✅ Active Subscribers by Registration Month" :
                 statusFilter === "INACTIVE" ? "⚠️ Inactive Subscribers by Registration Month" :
                                              "📈 All Subscribers by Registration Month"}
                {countryFilter !== "ALL" && ` — ${COUNTRY_NAMES[countryFilter]||countryFilter}`}
              </h3>
              <p style={{ margin:"0 0 4px", fontSize:11, color:"#64748b" }}>
                Each bar = users who registered that month and are currently{" "}
                <b style={{ color: statusFilter==="ACTIVE" ? "#0e9f6e" : statusFilter==="INACTIVE" ? "#e3a008" : "#1a56db" }}>
                  {statusFilter === "ACTIVE" ? "ACTIVE" : statusFilter === "INACTIVE" ? "INACTIVE (VERIFY + Suspended)" : "ALL statuses"}
                </b>
                {countryFilter !== "ALL" && ` in ${COUNTRY_NAMES[countryFilter]||countryFilter}`}
              </p>
              <p style={{ margin:"0 0 14px", fontSize:11, color:"#1a56db", fontWeight:700 }}>
                Total: {fmt(subMonthlyFiltered.reduce((s,m)=>s+m.new_subscribers,0))} users
              </p>
              <ResponsiveContainer width="100%" height={260}>
                <BarChart data={subMonthlyFiltered} margin={{ top:24, right:20, left:0, bottom:5 }}>
                  ...
                  <Tooltip content={({ active, payload, label }) => {
                    ...
                    return (
                      <div style={{ background:"#1e293b", borderRadius:8, padding:"10px 14px", fontSize:12, color:"#f8fafc" }}>
                        <div style={{ fontWeight:700, marginBottom:6, color:"#94a3b8" }}>{label}</div>
                        <div>✅ Active: <b>{d.active}</b></div>
                        <div>🔍 Verify: <b>{d.inactive}</b></div>
                        <div>📊 Total registered: <b>{d.total}</b></div>
                      </div>
                    );
                  }}/>
                  <Bar dataKey="new_subscribers" name="Subscribers" radius={[4,4,0,0]} fill="#0e9f6e">
                    <LabelList dataKey="new_subscribers" position="top" style={{ fontSize:10, fontWeight:700, fill:"#374151" }}/>
                    {subMonthlyFiltered.map((m,i)=>(
                      <Cell key={i} fill={
                        statusFilter==="ACTIVE"   ? "#0e9f6e" :
                        statusFilter==="INACTIVE" ? "#e3a008" :
                        "#1a56db"
                      }/>
                    ))}
                  </Bar>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:18, marginBottom:18 }}>
              <div style={{ background:"#fff", borderRadius:12, padding:20, boxShadow:"0 1px 4px rgba(0,0,0,0.06)" }}>
                <h3 style={{ margin:"0 0 16px", fontSize:14, fontWeight:700 }}>
                  {statusFilter === "ACTIVE"   ? "✅ Active Subscriber Breakdown" :
                   statusFilter === "INACTIVE" ? "⚠️ Inactive Subscriber Breakdown" :
                ...
                <ResponsiveContainer width="100%" height={220}>
                  <PieChart>
                    <Pie data={(() => {
                      if (statusFilter === "ACTIVE")   return [{ name:"Active", value:totalActive }];
                      if (statusFilter === "INACTIVE") return [{ name:"VERIFY", value:totalInactive }, { name:"Suspended", value:totalSuspended }];
                      return [{ name:"Active", value:totalActive }, { name:"VERIFY", value:totalInactive }, { name:"Suspended", value:totalSuspended }];
                    })()} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={85}
                      label={({name,percent})=>`${(percent*100).toFixed(0)}%`}>
                      {["#0e9f6e","#e3a008","#e02424"].map((c,i)=><Cell key={i} fill={c} />)}
                    </Pie>
